...
const main = async () => {
  ...
  // Unfortunately we can't just compare the titles or indicies, because different games work differently. We have to look for ANY change.
  const diff = recursiveDiff.getDiff(oldState, state.delta[state.index])
  if (!diff.length) { // No change since last poll.
    return end()
  } else {
    ...
    window.dispatchEvent(new CustomEvent('TwineChange', { detail: { index: state.index, state: state.delta[state.index], oldState, diff } }));
  }
  return end()
}
